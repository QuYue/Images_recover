...
scales = [0, 0] # 0表示采用算术刻度，1表示菜用对数刻度
# 调用函数创建译码矩阵
FieldD = ea.crtfld(Encoding, varTypes, ranges, borders,
                   precisions, codes, scales)

#%% ==========遗传算法参数设置==========
NIND = 20 # 种群个体数目
MAXGEN = 100 # 最大遗传代数
maxormins = [1] # 表示目标函数是最小化，元素为-1则是表示对应的目标函数是最大化
selectStyle = 'sus' # 采用随机抽样选择
recStyle = 'xovdp' # 采用两点交叉
mutStyle = 'mutbin' # 采用二进制染色体的变异算子
pc = 0.9 # 交叉概率
pm = 1 # 整条染色体的变异概率（每一位的变异概率=pm/染色体长度）
Lind = int(np.sum(FieldD[0, :])) # 计算染色体的长度
...
for gen in range(MAXGEN):
    ...
    SelCh = ea.mutate(mutStyle, Encoding, SelCh) # 变异
